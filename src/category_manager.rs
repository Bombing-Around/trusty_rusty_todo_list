...
impl<'a> CategoryManager<'a> {
    ...
    pub fn set_category_order(&mut self, category_id: u64, new_order: u32) -> Result<(), StorageError> {
        let mut data = self.storage.load()?;
        if let Some(category) = data.categories.iter_mut().find(|c| c.id == category_id) {
    ...
    pub fn reorder_categories(&mut self, category_ids: Vec<u64>) -> Result<(), StorageError> {
        ...
        for id in &category_ids {
            if !data.categories.iter().any(|c| c.id == *id) {
                return Err(StorageError::Storage(format!("Category with id {} not found", id)));
            }
        }
...
mod tests {
    ...
    fn test_delete_category() {
        ...
        let id = manager.add_category("Test".to_string(), None).expect("Failed to add category");
    ...
    fn test_category_ordering() {
        ...
        // Add categories
        let id1 = manager.add_category("A".to_string(), None).expect("Failed to add category");
        let id2 = manager.add_category("B".to_string(), None).expect("Failed to add category");
        let id3 = manager.add_category("C".to_string(), None).expect("Failed to add category");
        
        // Set custom order
        manager.set_category_order(id2, 1).expect("Failed to set order");
        manager.set_category_order(id1, 2).expect("Failed to set order");
        manager.set_category_order(id3, 3).expect("Failed to set order");
    ...
    fn test_default_category_order() {
        ...
        // Add categories
        let _id1 = manager.add_category("A".to_string(), None).expect("Failed to add category");
        let _id2 = manager.add_category("B".to_string(), None).expect("Failed to add category");
        let _id3 = manager.add_category("C".to_string(), None).expect("Failed to add category");
