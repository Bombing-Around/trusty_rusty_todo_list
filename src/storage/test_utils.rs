...
pub fn create_test_config_manager() -> (ConfigManager, TempDir) {
    ...
    let mut config = Config::default();
    let storage_path = temp_dir.path().join("test-data.json").to_str().unwrap().to_string();
    ...
    config_manager.set("storage.path", &storage_path).expect("Failed to set storage.path");
...
mod tests {
    use super::*;
    use crate::models::{Category, Task, Priority};
    
    #[test]
    fn test_storage_initialization() {
        let test_storage = TestStorage::new();
        let data = test_storage.storage().load().expect("Failed to load storage");
    ...
    fn test_category_operations() {
        ...
        storage.add_category(category.clone()).expect("Failed to add category");
    ...
    fn test_config_manager() {
        ...
        let storage_path = config_manager.get("storage.path").expect("Storage path not set");
