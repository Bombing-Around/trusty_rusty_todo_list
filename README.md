...
| Command                                                                                               | Description                                                                                                                               |
| ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `trt add <title> [--category <category_name or category_id> (or -c)] [--priority <high|medium|low>] [--description <text> (or -d)]` | Add a new task with the given title, optional priority, and optional description. Omitting `--category` uses the current category context, else `default-category`, else Uncategorized |
| `trt delete <title or id> [--category <category_name or category_id> (or -c)]`                     | Delete the task with the given title                                                                                                      |
| `trt update <title or id> [--to <new_title> (or -t)] [--category <category_name or category_id> (or -c)] [--description <text> (or -d)] [--clear-description]` | Update the task's title and/or description. At least one of `--to`, `--description`, `--clear-description` is required - `--clear-description` is the only way to blank an existing description, since an empty `--description ""` is not treated the same way |
| `trt show <title or id> [--category <category_name or category_id> (or -c)]`                       | Show full details of a task, including its description                                                                                    |
...
| `trt list [--search <term>] [--completed] [--priority <high|medium|low>] [--category <category_name or category_id>] [--all]` | List tasks with their IDs, optionally filtered. Narrows to the current category context if one is set, unless `--category` or `--all` says otherwise |
...
| `trt category add <name> [--description <text> (or -d)]`                                           | Add a new category with the given name and optional description                                                                          |
| `trt category delete <name> (--new-category <category_name or category_id>)`                       | Delete a category and optionally move its tasks                                                                                           |
| `trt category update <old_name> [<new_name>] [--description <text> (or -d)] [--clear-description]` | Rename a category and/or update its description. At least one of `<new_name>`, `--description`, `--clear-description` is required - `--clear-description` is the only way to blank an existing description, since an empty `--description ""` is not treated the same way |
| `trt category list`                                                                                | List all categories with their IDs and descriptions                                                                                        |
...
`list` honors the category context the same way every other task command does: with no `--category` and no `--all`, a `category use`d context narrows the listing to that one category, and the output says so with a "Tasks in category '\<name\>':" header instead of the plain "Tasks:" header used when nothing narrows it. An explicit `--category <category_name or category_id>` always wins and narrows to that category regardless of context. `--all` ignores the context entirely and lists every category; it cannot be combined with `--category`. Soft-deleted tasks never appear in `list`, narrowed or not.

Both tasks and categories carry an optional description, set with `--description` on `add`/`category add` and changed afterward with the same flag on `update`/`category update`. `update`/`category update` require at least one field to actually change - a new title/name, `--description`, or `--clear-description` - so an invocation naming none of them is refused rather than silently doing nothing. Clearing a description back to empty is deliberately a separate flag, `--clear-description`, rather than something an empty `--description ""` does: the two are kept apart so a description is never blanked by accident. A task's description is not shown in `list` - descriptions can run long and `list` is meant to be scanned - so `trt show <title or id>` exists to show one task's full detail, description included. A category's description, being short and one per category, is shown directly in `category list`; a category without one contributes no description text to its line at all, rather than a placeholder, since `category list` is also a scan.
